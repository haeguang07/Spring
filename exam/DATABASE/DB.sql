...
VALUES (100,'리눅스','100.jpg','15/09/02',24000,'나룩스','운영체제, DB기초, 네트위크기초, 개발환경구축');
INSERT INTO book_tbl_06(book_no, book_name, book_coverimg, book_date, book_price, book_publisher, book_info)
...
COMMIT;
delete from book_tbl_06 where book_no >= 105;



select a.book_no,c.book_name , rent_total,rent_count
from (  
        select book_no, sum(rent_price) rent_total
        from rent_tbl_06
        GROUP BY book_no) a JOIN
     (  select book_no, count(book_no) rent_count
        from rent_tbl_06
        GROUP BY book_no) b
